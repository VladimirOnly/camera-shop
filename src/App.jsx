        ...
        if (productIdFromUrl) {
          ...
          if (foundProduct) {
            ...
            if (PRODUCT_TABS.includes(foundProduct.category)) {
              ...
              setCurrentView("home");
            } else if (foundProduct.category === PORTFOLIO_TAB) {
    ...
    <div className="min-h-screen bg-[#F5F5F2] text-[#121826] font-sans flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-[#121826]/95 backdrop-blur-md border-b border-[#2A3441] shadow-xl">
        <div className="container mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer h-full py-2" onClick={() => handleNavToHome()}>
            <img src="/images/logo.png" alt="I SEE YOU 24" className="h-full w-auto object-contain" />
          </div>
          
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-6 mr-4">
              <button onClick={() => handleNavToHome('services')} className="text-xs font-bold uppercase tracking-widest text-[#9CA3AF] hover:text-white transition">Služby</button>
              <button onClick={handleNavToPortfolio} className="text-xs font-bold uppercase tracking-widest text-[#9CA3AF] hover:text-white transition">Naše práce</button>
            </div>
            <button onClick={() => {
               if (currentView === "home") handleNavToHome('contact');
               else document.getElementById('contact').scrollIntoView({ behavior: 'smooth' });
            }} className="text-xs font-bold uppercase tracking-widest text-[#E7E6E1] border border-[#E7E6E1]/30 px-6 py-2 hover:bg-[#E7E6E1] hover:text-[#121826] transition duration-300 rounded-sm">
              Poptávka
            </button>
          </div>
        </div>
      ...
      {/* --- ГЛАВНАЯ СТРАНИЦА (Каталог и Услуги) --- */}
      {currentView === "home" && (
        <>
          <div className="bg-[#121826] text-[#E7E6E1] py-24 px-4 relative overflow-hidden">
            <div className="container mx-auto text-center relative z-10 max-w-4xl">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <button onClick={() => handleNavToHome('services')} className="bg-[#E7E6E1] text-[#121826] px-8 py-4 font-bold uppercase tracking-widest hover:bg-white transition shadow-lg">
                  Naše Služby
                </button>
                <button onClick={handleNavToPortfolio} className="bg-[#1A2332] border border-[#2A3441] text-[#E7E6E1] px-8 py-4 font-bold uppercase tracking-widest hover:bg-[#2A3441] transition shadow-lg">
                  Naše práce
                </button>
                <button onClick={() => handleNavToHome('contact')} className="border border-[#E7E6E1]/30 text-[#E7E6E1] px-8 py-4 font-bold uppercase tracking-widest hover:bg-[#E7E6E1]/10 transition">
                  Kontaktovat
                </button>
              </div>
            </div>
      ...
      {currentView === "portfolio" && (
        <section id="portfolio" className="py-20 flex-grow bg-white">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-gray-500">Portfolio</span>
              <h1 className="text-4xl font-bold uppercase tracking-wide text-[#121826] mt-2">Naše práce</h1>
              <div className="w-20 h-1 bg-[#121826] mx-auto mt-4"></div>
            </div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#121826]/90 backdrop-blur-sm transition-opacity duration-300" onClick={() => setSelectedProduct(null)}>
          <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto relative flex flex-col md:flex-row shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 z-10 p-2 text-gray-400 hover:text-[#121826] transition bg-white/80 rounded-full">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>

            <div className="w-full md:w-1/2 bg-[#F5F5F2] flex items-center justify-center p-6 md:p-12">
